...
package org.financeapps.hermesmoneymanager.notifications;
...
import org.financeapps.hermesmoneymanager.settings.BehaviourSettings;
import org.financeapps.hermesmoneymanager.settings.GeneralSettings;
...
public class SmsReceiverTransactions extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
                        ...
                        if (transType != "" && msgBody.toLowerCase().contains("otp") == false) { // if not from blank, then nothing to do with sms
                            ...
                            if ((!fromAccountDetails[6].isEmpty() || !toAccountDetails[6].isEmpty() ||
                                    ...
                                    case 0: //add new trnsaction
                                        ...
                                        if (transType == "Transfer") //if it is transfer
                                        {
                                            if (!toAccountDetails[0].isEmpty()) // if id exists then considering as account transfer
                                            {
                                                ...
                                                //if there is no to account found from hmm db, then check for payee
                                ...
                                if(skipSaveTrans == false) {
                                    if (validateData()) {
                                        if (saveTransaction()) {
                                            ...
                                            {
                                                Toast.makeText(context, "hmm: Bank Transaction Processed for: \n\n" + strExtracted, Toast.LENGTH_LONG).show();
                                            }
                                        }
                                        else
                                        {
                                ...
                                else
                                {
                                    if (behav_settings.getSmsTransStatusNotification()==true)
                                    {
                                        showNotification(t_intent, "hmm: Skiping Bank Transaction updates SMS, because transaction exists with ref. no. " + transRefNo, msgSender, "Already Exists");
                                    }
                                    else
                                    {
                                        Toast.makeText(context, "hmm: Skiping Bank Transaction updates SMS, because transaction exists with ref. no. " + transRefNo, Toast.LENGTH_LONG).show();
                                    }
                                }
        ...
        {
            Timber.e(e, "hmm: Bank Transaction Process EXCEPTION");
        }
    }
    ...
    public boolean validateData() {
        ...
        if (mCommon.transactionEntity.getAccountId() == Constants.NOT_SET) {
            //Toast.makeText(mContext, "hmm : " + (R.string.error_toaccount_not_selected), Toast.LENGTH_LONG).show();
            return false;
        }
        ...
        if (isTransfer) {
            ...
            if (mCommon.transactionEntity.getAccountToId().equals(mCommon.transactionEntity.getAccountId())) {
                //Toast.makeText(mContext, "hmm : " + (R.string.error_transfer_to_same_account), Toast.LENGTH_LONG).show();
                return false;
            }

            // Amount To is required and has to be positive.
            if (this.mCommon.transactionEntity.getAmountTo().toDouble() <= 0) {
                //Toast.makeText(mContext, "hmm : " + (R.string.error_amount_must_be_positive), Toast.LENGTH_LONG).show();
                return false;
            }
        } else{
        ...
        if (!hasCategory && !isTransfer) {
            //Toast.makeText(mContext, "hmm : " + (R.string.error_category_not_selected), Toast.LENGTH_LONG).show();
            return false;
        }
    ...
    private void showNotification(Intent intent, String notificationText, String msgSender, String txnStatus) {
        ...
        try {
            ...
            String GROUP_KEY_Ahmm = "com.android.example.Hermes Moneymanager";
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                ...
                NotificationChannel nChannel = new NotificationChannel(CHANNEL_ID, "AhmmSMS", NotificationManager.IMPORTANCE_DEFAULT);
